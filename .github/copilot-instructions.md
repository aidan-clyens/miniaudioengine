...
## Code Style
- C++20 required; keep layer-specific namespaces (`miniaudioengine::core`, `::data`, `::control`, `::processing`, `::file`).
...
## Architecture
- Strict layered system: framework (Layer 0) -> data (Layer 1, real-time) -> processing (Layer 2, partial/experimental) -> control (Layer 3, synchronous) -> cli/examples (Layer 4).
- Control plane is synchronous singletons (main thread, locks allowed) in `src/control/` (e.g., `AudioStreamController`, `MidiPortController`, `TrackManager`, `DeviceManager`, `FileManager`).
- Data plane executes in RtAudio/RtMidi callbacks in `src/data/` and must be lock-free and allocation-free; use `LockfreeRingBuffer<T, Size>` from `src/framework/include/lockfree_ringbuffer.h`.
- Data plane components: `AudioDataPlane`, `MidiDataPlane`, `AudioCallbackHandler`, `MidiCallbackHandler`.
- Processing plane components are minimal: `IAudioProcessor`, `Sample`, `SamplePlayer` (no worker orchestration yet).
...
- Legacy `IEngine<T>` threading + `MessageQueue<T>` exist in `src/framework/include/engine.h` but new control components should not use them.
- `IAudioController` is deprecated; prefer `AudioStreamController` directly.
